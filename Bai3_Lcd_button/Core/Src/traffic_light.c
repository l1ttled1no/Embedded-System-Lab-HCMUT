			...
			if (isLightOn) {
				...
				switch (systemState) {
					...
					case MODIFY_YELLOW_LIGHT:
						...
						lcd_ShowIntNum(140, 105, temp_yellow, 2, WHITE, RED, 32);
						break;
					default:
						break;
				}
...
void handleConfirmValue(void) {
    ...
    if (temp_yellow == 0){
    	temp_yellow = 1;
    	if (temp_green == 0){
    		temp_green = temp_red - temp_yellow;
    	}
    	else temp_yellow = temp_red - temp_green;
    }
